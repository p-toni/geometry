...
export function clampItemPosition(col: number, row: number, cols: number, rows: number) {
  return {
    col: clamp(snapToCell(col), 0, GRID_COLS - cols),
    row: clamp(snapToCell(row), 0, GRID_ROWS - rows),
  };
}

export function clampItemSize(cols: number, rows: number, col: number, row: number) {
  return {
    cols: clamp(snapToCell(cols), 1, GRID_COLS - col),
    rows: clamp(snapToCell(rows), 1, GRID_ROWS - row),
  };
}
